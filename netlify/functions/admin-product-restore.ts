...
import { isSafeProductSlug, productPathCandidates, restoreProductMarkdown } from './product-archive-core';
...
export const handler: Handler = async (event) => {
  ...
  try {
    const restored = restoreProductMarkdown(current);
    if (restored.alreadyActive) {
      return { statusCode: 409, body: JSON.stringify({ error: `${restored.title} is already active.` }) };
    }
    ...
    return {
      ...
      body: JSON.stringify({
        pendingChange: {
          ...
          proposal_id: `admin-restore-${Date.now()}-${randomUUID()}`,
          judgeDecision: 'allow',
          risk_flags: ['owner_confirmed_restore'],
        },
      }),
    };
  } catch (error) {
    return {
      ...
      body: JSON.stringify({ error: error instanceof Error ? error.message : 'Could not restore product' }),
    };
  }
};

...
export const handler: Handler = async event => {
  ...
  try {
    ...
    const proposal = normaliseVoiceProposal({ ...capture.proposal, summary: clean(body.summary, 2400) || capture.proposal.summary, customerName: clean(body.customerName, 180) || capture.proposal.customerName, productInterest: clean(body.productInterest, 240) || capture.proposal.productInterest, followUpDate: clean(body.followUpDate, 10), followUpReason: clean(body.followUpReason, 500) || capture.proposal.followUpReason });
    ...
    const applied = { ...capture, status: 'applied' as const, proposal, confirmedAt: now, appliedAt: now, savedEnquiryId: enquiryId, savedCustomerId: customerId };
    ...
    return json(200, { ok: true, capture: applied, result: { enquiryId, customerId, followUpDate: proposal.followUpDate, message: 'Call note saved. Any prices, agreement changes, deposits, and production decisions still require the normal reviewed workflow.' } });

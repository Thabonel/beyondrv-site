...
import { CREATABLE_KINDS, EVENT_KIND_META, TIMED_RECORD_KINDS, type AdminCalendarEvent } from './calendar-model';
...
export default function EventForm({ mode, initial, event, orders, onLoadOrders, onSubmit, onCancel }: Props) {
  ...
  useEffect(() => {
    if (values.kind === 'customer_visit' && !orders.length) onLoadOrders();
  ...
  async function submit(formEvent: React.SyntheticEvent<HTMLFormElement>) {
    ...
    setError('');
    if (!editingRecord && !values.title.trim()) { setError('Give it a title.'); return; }
    if (values.kind === 'customer_visit' && mode === 'create' && !values.orderId) { setError('Choose the order the customer is visiting for.'); return; }
  ...
  return (
    <form className="gcal-form" onSubmit={submit} data-testid="event-form">
      ...
      {values.kind === 'customer_visit' && mode === 'create' && (

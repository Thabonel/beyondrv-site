...
export default function ContractManager({ products, customers, leads, initialContractId = '', newAgreementRequest = 0 }: { products: ProductOption[]; customers: CustomerOption[]; leads: LeadOption[]; initialContractId?: string; newAgreementRequest?: number }) {
  ...
  return (
    <div style={{ padding: '1rem', overflowY: 'auto', display: 'grid', gap: '0.9rem' }}>
      ...
      {draft.salesContext?.source === 'website_enquiry' && Boolean(draft.salesContext?.enquiryMessage || draft.salesContext?.statedProductInterest) && <section data-testid="website-enquiry-context" style={{ background: '#201a12', border: '1px solid #92400e', borderRadius: '8px', padding: '0.85rem', display: 'grid', gap: '0.5rem' }}>

...
1. Owner opens `/admin`
2. Types instructions: "Drop the Sunpatch price to $74k", "Add a $5k discount", "Mark the 3.5m cabover as coming in the next container with a March ETA", etc.
...
- Claude tools: `read_file(path)` fetches file content from GitHub API, `list_files(dir)` lists files in a repo directory, `propose_change(path, content, description)` queues a change without committing, and `remove_change(path)` dequeues a change.
...
- Add or remove discount labels
- Update public availability and ETA wording
- Toggle online purchase eligibility
- Toggle stock status / sale status

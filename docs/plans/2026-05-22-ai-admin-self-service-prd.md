...
- Let the owner update product prices, descriptions, status, featured flags, and general website copy from the admin page.
...
- Add clear owner instructions for common tasks:
  - Update product price.
  - Update product copy/specs.
  - Mark a product as sold or remove it.
  - Add a basic product entry.
  - Add business knowledge for the public chatbot.
...
- Update the admin AI prompt so it understands:
  - Product markdown location.
  - Chatbot knowledge file location.
  - Supported product status values.
  - Current image-upload limitation.
  - Sold product handling.
...
- Product list with current title, price, category, status, hero image, and featured state.
- Guided forms for:
  - Add product.
  - Edit product.
  - Remove product.
...
- Product status must be limited to supported values.
...
- Image storage strategy:
  - Option A: GitHub repository assets.
  - Option B: Cloudinary.
  - Option C: Netlify Blobs or S3-compatible storage.

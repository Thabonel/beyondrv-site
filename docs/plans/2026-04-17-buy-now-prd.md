...
This PRD now uses the shared product/admin model:

- `price` is the current sale price shown to the customer.
- `compareAtPrice` is display-only and represents the previous or reference price.
- `saleLabel` is optional display copy such as "Clearance" or "Special".
- `availability` controls whether Stripe checkout is exposed.
- `leadTimeText` and `containerEtaText` are public display fields only.
...
- Shared availability gating via Markdown frontmatter and the admin model
...
| Public availability | Deposit Button | Buy Now Button | Finance Panel |
|--------------------|---------------|----------------|---------------|
| `available_in_australia` | Active if deposit is enabled | Active if full payment is enabled | Active |
| `coming_next_container` | Hidden unless explicitly enabled | Hidden unless explicitly enabled | Active or hidden by owner choice |
| `made_to_order` | Hidden unless explicitly enabled | Hidden unless explicitly enabled | Active or hidden by owner choice |
| `ask_availability` | Replaced by "Ask about availability" badge | Hidden | Hidden |
| `unavailable` | Replaced by "Unavailable" badge | Hidden | Hidden |
...
- The public current price remains the source of truth for Stripe calculations; compare-at/original price and sale labels are display-only.
...
3. Reject if availability does not permit checkout or if the product is not marked purchasable online
...
Below the price on each product card; only when availability and purchase eligibility allow it:
...
- Sold / On-Order: replace the entire group with a single availability badge
...
│   ├── content/products/*.md              ← source of truth (price, availability, sale fields)
...
Customer browser
   ...
   │                                              ├─ verify availability / purchase eligibility
...
price: "$78,999"
availability: available_in_australia  # available_in_australia | coming_next_container | made_to_order | ask_availability | unavailable
compareAtPrice: "$84,999"
saleLabel: Clearance
slug: sunpatch-15-xc
...
---
...
At build time, Astro generates `dist/products.json` (manifest of slug → price, compare-at price, sale label, and availability) which the checkout Function loads. Toggling availability or pricing via AI admin commits markdown → Netlify rebuild → manifest updated → buttons and labels refresh within ~30s.
...
- Schema: `{ slug, name, price, compareAtPrice?, saleLabel?, availability }`
- Functions read the bundled file at cold start

### Task 3 — `/api/checkout` Function
- TypeScript Netlify Function
- Verify Turnstile token
- Lookup price and availability from manifest, reject if checkout is not permitted
...
- Props: `slug`, `name`, `price`, `availability`, `compareAtPrice?`, `saleLabel?`
- Render compact (card) or full (PDP) variant via prop
- Compute deposit display from `site-config.yaml.deposit_percent`
- Availability badges for sold / on-order / coming-soon states
...
|------|-----------|--------|------------|
| Customer pays deposit for already-sold unit | Low | High | Server-side availability check in `/api/checkout`; badge replaces button on UI |
